...
import CreateRole from "@/views/master/role/create-role"
import UpdateRole from "@/views/master/role/update-role"
...
export default {
    components: { Layout, PageHeader, TableHeadPagination, CreateRole, UpdateRole},
    data() {
        const columnData = [
            ...
            { width: "auto", label: "Aksi", name: "menu_master_icon" },
        ]
        const sortOrderData = [];
        ...
        return {
            ...
            tableData: {
                // init table data
                UrlDataTable: process.env.VUE_APP_BACKEND_URL_API + 'roles',
                DrawTable: 0,
                LoadingTable: true,
                DataTable:[],
    ...
    methods: {
        ...
        dataTableAction(tableData){
            ...
            this.tableData.UrlDataTable = tableData.LinkPagination ? tableData.LinkPagination : process.env.VUE_APP_BACKEND_URL_API + 'roles';
            this.getTableData();
        },
        ...
        changeStatus(status, data){
            ...
            Swal.fire(notification.swalConfirmationAttr).then((result) => {
                if (result.isConfirmed) {
                    ...
                    let config_status = {
                        method: "put",
                        url: process.env.VUE_APP_BACKEND_URL_API + 'roles/' + data?.role_id + '/' + status,
                        headers: {
                            Authorization: "Bearer " + localStorage.getItem('accessToken')
                        }
                    }
        ...
        deleteData(data){
            ...
            Swal.fire(notification.swalConfirmationAttr).then((result) => {
                if (result.isConfirmed) {
                    ...
                    let config_delete = {
                        method: "delete",
                        url: process.env.VUE_APP_BACKEND_URL_API + 'roles/' + data?.role_id,
                        headers: {
                            Authorization: "Bearer " + localStorage.getItem('accessToken')
                        }
                    }
...
<template>
    <Layout>
        <PageHeader title="Role" pageTitle="Master" />
        <BRow>
            <BCol cols="12">
                <BCard no-body>
                    <BCardBody class="p-4">
                        <div class="row">
                            <div class="col-12">
                                <BCardTitle>Filter Data Role</BCardTitle>
                                <hr/>
                            </div>
                            <div class="col-12">
                                <div class="row">
                                        ...
                                        <div v-if="data.status == 'ENABLE'">
                                            <div class="d-flex flex-wrap justify-content-center gap-2">
                                                ...
                                                <button type="button" class="btn btn-primary btn-sm">
                                                    <i class="bx bx-transfer font-size-16 align-middle me-1"></i> Hak Akses
                                                </button>
    ...
    <!-- Modal Tambah Data -->
    <BModal v-model="modalFormShowAdd" id="modal-add" size="lg" title="Tambah Role" title-class="font-18" hide-footer>
        <CreateRole v-if="modalFormShowAdd" @closeFormRefreshTable="closeFormRefreshTable"></CreateRole>
    </BModal>

    <!-- Modal Edit Data -->
    <BModal v-model="modalFormShowEdit" id="modal-edit" size="lg" title="Edit Role" title-class="font-18" hide-footer>
        <UpdateRole v-if="modalFormShowEdit" :dataEdit="editedData" @closeFormRefreshTable="closeFormRefreshTable"></UpdateRole>
    </BModal>
</template>